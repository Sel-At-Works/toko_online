...
elseif ($aksi === 'delete') {
    ...
    // Hapus transaksi_penjual
    mysqli_query($conn, "DELETE FROM transaksi_penjual WHERE id = $tp_id AND penjual_id = $penjual_id");

    // Hapus transaksi_detail terkait transaksi ini
    mysqli_query($conn, "DELETE FROM transaksi_detail WHERE transaksi_id = $transaksi_id");

    // Hapus transaksi utama jika sudah tidak ada penjual tersisa
    $cek = mysqli_query($conn, "SELECT COUNT(*) AS jumlah FROM transaksi_penjual WHERE transaksi_id = $transaksi_id");
